...
\Large December 2016 \\
\normalsize Document v1.6 \\
\normalsize Library v7.2
\end{flushright}
...
Although the \emph{libMeshb} still handles versions 1 and 2 for the sake of compatibility, it is strongly advised to create version 3 files since more and more computer are 64-bit capable.
...
The 64 bits integer data type used by the library is the \emph{long int}, which may be set to 32 bits by come compilers. In this case, the \emph{libMeshb} will simply return with an error when dealing with version 4 mesh files. Most compilers, GNU and Intel for instance, set the long int size to 64 bits unless you specify the "-m32" option.
